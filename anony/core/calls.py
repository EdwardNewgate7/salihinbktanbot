...
class TgCall(PyTgCalls):
    ...
    async def play_media(
        ...
        if isinstance(media.file_path, str) and media.file_path.startswith(("http://", "https://")):
            extra_ffmpeg = config.STREAM_FFMPEG
        if config.FFMPEG_PARAMS:
            extra_ffmpeg = f"{extra_ffmpeg} {config.FFMPEG_PARAMS}".strip()
        seek_flag = f"-ss {seek_time}" if seek_time > 1 else ""
        ffmpeg_params = f"{seek_flag} {extra_ffmpeg}".strip() or None
    ...
    async def boot(self) -> None:
        ...
        for ub in userbot.clients:
            client = PyTgCalls(ub, cache_duration=config.STREAM_CACHE)
            await client.start()
