...
class MongoDB:
    ...
    async def get_welcome(self, chat_id: int) -> bool:
        if chat_id not in self.welcome:
            doc = await self.chatsdb.find_one({"_id": chat_id})
            if doc and doc.get("welcome"):
                self.welcome.append(chat_id)
        return chat_id in self.welcome

    async def set_welcome(self, chat_id: int, status: bool = False) -> None:
        if status:
            if chat_id not in self.welcome:
                self.welcome.append(chat_id)
        else:
            if chat_id in self.welcome:
                self.welcome.remove(chat_id)
        await self.chatsdb.update_one(
            {"_id": chat_id},
            {"$set": {"welcome": status}},
            upsert=True,
